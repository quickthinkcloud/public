...
New-ItemProperty -Path $winLogonKey -Name "AWSAccessKey" -Value "aa" -ErrorAction SilentlyContinue 
New-ItemProperty -Path $winLogonKey -Name "AWSSecretKey" -Value "aa/aa" -ErrorAction SilentlyContinue 
START http://boxstarter.org/package/url?https://qtcloud.box.com/shared/static/ni1650m10790zjafb79yhq7fmnjr6iq9.ps1
#>
...
#
# Libre
$softwareName = "LibreOffice_5.3.6_Win_x64"
$softwareFilename = "LibreOffice_5.3.6_Win_x64.msi"
$softwareFolderInQTCsoftwareREPO = "LibreOffice" # no trailing slash
if (!(Test-Path "C:\Repository\$($softwareName)InstallAttempted.txt")) {
    MSIDownloadAndInstall
    if (Test-PendingReboot) { Invoke-Reboot }
} else {
    Add-Content -path "C:\Repository\$($softwareName)InstallAttempted.txt" -value "$(Get-Date -format 'dd/MM/yyyy HH:mm:ss') - Detected previous installation attempt - aborting. `n"
    Write-Host "File already exists and new text content added"
